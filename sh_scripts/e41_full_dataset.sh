...
#SBATCH --job-name=e41_full_dataset
...
#SBATCH --output=/cluster/work/projects/nn8106k/%u/github/LA-SIE/slurm/e41_full_dataset_%j.out
#SBATCH --error=/cluster/work/projects/nn8106k/%u/github/LA-SIE/slurm/e41_full_dataset_%j.err
...
RUN="${RUN:-$WORK/runs/e41_full_dataset_$(date +%Y%m%d_%H%M%S)}"
...
export WANDB_PROJECT="${WANDB_PROJECT:-e41_full_dataset}"
...
if [ "$SIZE_DATASET" = "-1" ]; then
  echo "Using the full dataset; steps_per_epoch and total_steps depend on dataset length at runtime"
else
...
if [ "$ENABLE_WANDB" = "1" ]; then
  MAIN_ARGS+=(--wandb --wandb-name "E41_full_dataset")
fi
